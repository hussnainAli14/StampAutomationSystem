...
import { codeToast, signuperrorToast } from "../../ApiService/ToastDisplay";
...
console.log(formik.values)
  const  getCode =async (email)=>{
    ...
    const res = await axios.post("/api/v1/users/sendVerificationCode",{"email":email})
  ...
  const getData = async (event)=>{
    ...
    if(res.data.status ==="success"){
      console.log('success')
      // localStorage.setItem('EMAIL',totalData.email)
      // totalData.email = 'rockeykhan142@gmail.com'
    getCode('rockeykhan142@gmail.com');
    }
   }
   catch(error){
     signuperrorToast();
